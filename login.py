...
def login():
    ...
    while True:
        ...
        if event =="-login-":
            ...
            cur.execute("SELECT ID FROM student WHERE nickname=? AND password=?",(name,surname,))
            if cur.rowcount == 0:
                cur.execute("SELECT ID FROM teacher WHERE nickname=? AND password=?",(name,surname,))
                if cur.rowcount == 0:
